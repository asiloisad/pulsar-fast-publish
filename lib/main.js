...
module.exports = {
  ...
  async gitPrepare(cwd, version) {
    ...
    try {
      ...
      atom.notifications.addError(`Fast Publish: failed`, { detail: err.stderr || err.stdErr || err.message, dismissable: true });
    }
  },
  ...
  async publish(dirPath, mode) {
    ...
    try {
      ...
      atom.notifications.addInfo(`Fast Publish: version updated from v${oldVersion} to v${newVersion}`);
      await this.gitPrepare(dirPath, newVersion);
    } catch (err) {
      atom.notifications.addError("Fast Publish: failed", { detail: err.message, dismissable: true });
    }
  },
  ...
  async ppmPublish(dirPath, mode) {
    ...
    try {
      ...
      if (tag) {
        atom.notifications.addInfo(`PPM Publish: use "PPM Publish > last Tag" to retry publishing ${tag}`);
      }
    }
  },
  ...
  async gitPush(cwd) {
    ...
    try {
      ...
      atom.notifications.addError(`Fast Publish: failed to push ${pkgName}`, { detail: err.stderr || err.stdErr || err.message, dismissable: true });
    }
  },
};
