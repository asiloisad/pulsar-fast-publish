...
export default {
  ...
  gitPrepare(cwd, version) {
    exec(`git add --all`, { cwd:cwd }, (error, stdout, stderr) => {
      if (error) {
        console.log('error: git-add', stderr)
        return
      } else {
        console.log('pass: git-add', stdout)
      }
      exec(`git commit --all -m "Prepare v${version} release"`, { cwd:cwd }, (error, stdout, stderr) => {
        if (error) {
          console.log('error: git-commit', stderr)
          return
        } else {c
          onsole.log('pass: git-commit', stdout)
        }
        exec(`git tag -a v${version} -m "Prepare v${version} release"`, { cwd:cwd }, (error, stdout, stderr) => {
          if (error) {
            console.log('error: git-tag', stderr)
            return
          } else {
            console.log('pass: git-tag', stdout)
          }
          exec(`git push origin --tags`, { cwd:cwd }, (error, stdout, stderr) => {
            if (error) {
              console.log('error: git-push', stderr)
              return
            } else {
              console.log('pass: git-push', stdout)
            }
